---
title: "笔记｜扩散模型（五）：Classifier-Free Guidance 理论推导与代码实现"
author: "LittleNyima"
source_url: https://zhuanlan.zhihu.com/p/712821377
platform: zhihu
collection: "学习收藏"
...
> 作者: LittleNyima | 来源: https://zhuanlan.zhihu.com/p/712821377

---

本文原文以 CC BY-NC-SA 4.0 许可协议发布于 笔记｜扩散模型（五）：Classifier-Free Guidance 理论与实现，转载请注明出处。
...
现在我们不希望使用分类器来计算 \nabla{\mathbf{x}_t}\log p(y|\mathbf{x}_t) 这一项，因此需要用另一种方式来表示这一项，把第一个公式中的这一项移到等号左侧：

\nabla_{\mathbf{x}_t}\log p(y|\mathbf{x}_t)=\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t|y)-\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t)

然后再代入第二个含有参数 s 的公式：

\begin{aligned} \nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t|y)&=\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t)+s\nabla_{\mathbf{x}_t}\log p(y|\mathbf{x}_t)\\ &=\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t)+s\left(\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t|y)-\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t)\right)\\ &=\underbrace{(1-s)\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t)}_{\textrm{unconditional}~\textrm{score}}+\underbrace{s\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t|y)}_{\textrm{conditional}~\textrm{score}} \end{aligned}
...
不过这个方法也有一些不容易理解的地方，观察一下公式可以发现 \nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t|y) 这一项同时出现在了等式的左侧和右侧，那么如果想学习 \nabla{\mathbf{x}_t}\log p(\mathbf{x}_t|y) ，直接对这一项进行学习就可以了，为什么还需要拆分成无条件生成和有条件生成两部分呢？根据我个人的理解，从上边的式子可以看出无条件生成是有条件生成的基础，生成的质量和多样性是由无条件生成的分数保证的，如果只有有条件生成而没有无条件生成，那么生成效果可能不佳。
...
2. Multi-head attention mechanism: “queries”, “keys”, and “values,” over and over again
3. 扩散模型（三）| Classifier Guidance & Classifier-Free Guidance
4. diffusion model（四）——文生图diffusion model（classifier-free guided）
...
![图片](https://pic1.zhimg.com/v2-15a7a2684d6e6c154641cb3c8eb8e657_l.jpg?source=32738c0c&needBackground=1)

![图片](https://pic1.zhimg.com/v2-5a9b96b3e5503909f5875645b490c87c_1440w.jpg)

![图片](https://pic3.zhimg.com/v2-30cd4be480d618d96be54a9bc0457a70_1440w.jpg)

![图片](https://pic4.zhimg.com/v2-4f095210024e20939e31b3f7f0df9a53_1440w.jpg)

![图片](https://pica.zhimg.com/v2-c5be1695771c4f9b442b5bde56e5e8e0_720w.jpg?source=172ae18b)

![图片](https://picx.zhimg.com/v2-15a7a2684d6e6c154641cb3c8eb8e657_l.jpg?source=172ae18b)

![图片](https://pic1.zhimg.com/v2-4277ea170ca71ab7267e6fe0f473e446.webp?source=7e7ef6e2&needBackground=1)

![图片](https://pic4.zhimg.com/v2-471c53ec1fb6db37bc89f1107033eff7.webp)

![图片](https://pic1.zhimg.com/v2-50ed67839273098f6d0e9c58f42dd3dc_250x0.jpg?source=172ae18b)

![图片](https://picx.zhimg.com/v2-52d511dc43f073b5b9c317b7b2daedc9_250x0.jpg?source=172ae18b)

![图片](https://pic1.zhimg.com/v2-b00aa4e50ee8f86393ef60c8c74a70aa_250x0.jpg?source=172ae18b)
